...
def advanced_settings_panel():
    ...
    with tab1:
        ...
        discoveries = [
            {"strategy": "Weather Underdog System", "edge": "+3.2%", "confidence": "HIGH", "status": "Testing"},
            {"strategy": "Divisional Fade", "edge": "+2.1%", "confidence": "MEDIUM", "status": "Validated"},
            {"strategy": "Primetime Home Dog", "edge": "+1.8%", "confidence": "MEDIUM", "status": "Testing"}
        ]
        
        for disc in discoveries:
            with st.expander(f"{disc['strategy']} - Edge: {disc['edge']}", expanded=False):
                col1, col2, col3 = st.columns(3)
    ...
    with tab2:
        ...
        with col4:
            if st.button("📊 Compare Models", width='stretch', help="A/B test models"):
                ...
                comparison = pd.DataFrame({
                    'Model': ['Current XGBoost', 'New XGBoost', 'LightGBM', 'Ensemble'],
        ...
        if auto_retrain:
            ...
            st.success(f"✅ Auto-retrain scheduled: Every {retrain_day} at {retrain_time}")
    ...
    with tab3:
        ...
        categories = {
            ...
            'Rest Days': ['rest_days_home', 'rest_days_away', 'is_back_to_back_home', 'is_back_to_back_away', 'post_bye_home', 'post_bye_away'],
            'Weather': ['temp', 'wind', 'is_dome', 'is_cold', 'is_windy'],
            'Form': ['win_pct_home', 'win_pct_away', 'point_diff_home', 'point_diff_away'],
            'Advanced': ['referee_penalty_rate', 'home_field_advantage', 'divisional_game']
        }
        ...
        for category, features in categories.items():
            with st.expander(f"{category} ({len(features)} features)", expanded=False):
                for feature in features:
                    ...
                    with col1:
                        enabled = st.checkbox(feature, value=True, key=f"feat_{feature}")
                    with col2:
...
3. sqrt(total_line) / wind_speed - Importance: 0.053
    ...
    with tab5:
        ...
        tasks = [
            {"name": "Daily Predictions", "schedule": "9:00 AM ET", "status": "Active", "last_run": "1 hour ago"},
            {"name": "Weekly Retrain", "schedule": "Monday 3:00 AM", "status": "Active", "last_run": "2 days ago"},
            {"name": "Data Sync", "schedule": "Every 6 hours", "status": "Active", "last_run": "3 hours ago"},
            {"name": "Bulldog Discovery", "schedule": "Daily 2:00 AM", "status": "Paused", "last_run": "Never"}
        ]
...
APPLE_CLIENT_ID: Not configured
        ...
        production_mode = st.toggle("Production Mode", value=True, help="Disable experimental features")
