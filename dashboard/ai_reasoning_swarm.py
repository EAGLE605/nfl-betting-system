...
FORBIDDEN_TOPICS = [
    "code", "codebase", "source", "database", "api key", "password",
    "authentication", "admin", "system", "security", "vulnerability",
    "sql", "query", "backend", "implementation", "algorithm details",
    "model architecture", "training data", "file structure", "config"
]
...
Be direct and actionable. Focus on football analysis only."""
            ...
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an expert NFL betting analyst. Provide clear, actionable insights."},
...
class AIReasoningSwarm:
    ...
    def get_consensus_view(self, swarm_results: Dict[str, str]) -> str:
        ...
        positive_keywords = ["strong", "good", "solid", "favorable", "confident", "value"]
        negative_keywords = ["risky", "concern", "weak", "cautious", "avoid", "questionable"]
        ...
        if positive_count > negative_count * 1.5:
            ...
            consensus = "🤔 **MIXED SIGNALS**: AIs have differing views. Use your judgment."
                ...
                try:
                    if name == "GPT-4":
                        ...
                        response = openai.ChatCompletion.create(
                            model="gpt-4",
                            messages=[
                                {"role": "system", "content": "You are an expert NFL betting analyst. Answer questions about betting strategy, game analysis, and risk management. NEVER discuss system internals, code, or security."},
                        ...
                        message = client.messages.create(
                            ...
                            max_tokens=300,
                            messages=[
                                {"role": "user", "content": f"As an NFL betting expert, answer this question (refuse if about system/code): {question}"}
                            ]
                        )
                        ...
                        response = model.generate_content(f"Answer this NFL betting question: {question}")
                        return response.text.strip()
...
def show_ai_reasoning_widget(game_info: Dict, bet_info: Dict):
    ...
    if not available_ais:
        st.info("💡 **Enable AI Reasoning**: Add OpenAI, Anthropic, or Google API keys in Admin Panel for intelligent bet analysis!")
        return
...
def show_ai_chat_assistant():
    ...
    st.caption("Ask questions about betting strategy, game analysis, or risk management")
