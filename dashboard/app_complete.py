...
def login_page():
    ...
    with col2:
        ...
        with tab2:
            ...
            password = st.text_input("Password (min 8 characters)", type="password", key="signup_pass")
...
def show_performance_page(user):
    ...
    with col4:
        ...
        st.metric("Current Bankroll", f"${current_bankroll:.2f}", delta=f"+${total_profit:.2f}")
...
def show_settings_page(user):
        ...
        if user['tfa_enabled']:
            ...
            if st.button("Enable 2FA"):
                ...
                uri = totp.provisioning_uri(
                    name=user['email'],
                    issuer_name="NFL Edge Finder"
                )
                
                import qrcode
                qr = qrcode.make(uri)
                
                st.success("✅ 2FA enabled! Scan this QR code with your authenticator app:")
    ...
    with tab3:
        ...
        min_prob = st.slider(
            ...
            0.50, 0.70, settings['min_probability'],
            step=0.01,
            format="%.0f%%"
        )
