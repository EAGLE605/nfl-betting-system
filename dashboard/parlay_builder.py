...
def get_roast(num_legs: int) -> str:
    ...
    for category in PARLAY_ROASTS:
        if category["legs"] == num_legs or (
            category["legs"] == "13+" and num_legs >= 13
        ):
...
def calculate_parlay_odds(legs: List[Dict]) -> Dict:
    ...
    return {
        ...
        "house_edge": (
            ((true_fair_multiplier - payout_multiplier) / true_fair_multiplier * 100)
            if true_prob > 0
            else 100
        ),
...
def show_parlay_builder():
    ...
    if st.button("💰 Calculate Parlay", type="primary", width="stretch"):
        ...
        bet_amount = st.number_input(
            "Bet amount ($):", min_value=1.0, max_value=1000.0, value=10.0, step=1.0
        )
        ...
        with col2:
            st.metric(
                "You Win", f"${potential_win:.2f}", delta=f"+${potential_win:.2f}"
            )
...
__all__ = ["show_parlay_builder", "calculate_parlay_odds", "get_roast"]
