...
from utils.feature_selection import (analyze_feature_correlations,
...
def main():
    parser = argparse.ArgumentParser(description="Analyze feature correlations and importance")
    parser.add_argument("--features", type=str, default="data/processed/features_2016_2024_improved.parquet",
                       help="Path to features file")
    parser.add_argument("--output", type=str, default="reports/feature_analysis.csv",
    ...
    exclude_cols = [
        ...
        "stadium_id", "stadium", "roof", "surface",  # Original categoricals
    ...
    corr_pairs = analyze_feature_correlations(X, numeric_features, threshold=args.correlation_threshold)
    
    if len(corr_pairs) > 0:
        logger.info(f"Found {len(corr_pairs)} highly correlated pairs (>={args.correlation_threshold}):")
    ...
    non_redundant = remove_redundant_features(X, numeric_features, 
    ...
    logger.info(f"  - Use these features for model training")
