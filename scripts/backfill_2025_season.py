...
for game in sample_2025_games:
    ...
    results.append(
        {
            ...
            "clv": clv,
            "cumulative_max": max(
                [r["bankroll"] for r in results] + [STARTING_BANKROLL]
            ),
    ...
    print(
        f"{status} {game['date']} - {game['away']} @ {game['home']}: "
...
if len(new_bets_df) > 0:
    ...
    new_bets_df["drawdown"] = (
        new_bets_df["bankroll"] - new_bets_df["cumulative_max"]
    ) / new_bets_df["cumulative_max"]
