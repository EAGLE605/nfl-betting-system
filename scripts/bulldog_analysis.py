...
class BulldogAnalysis:
    ...
    def __init__(
        ...
        if results_path:
            ...
            if results_path.exists():
                ...
                logger.warning(
                    "Backtest results not found - some analyses will be skipped"
                )
    ...
    def analyze_feature_importance(self) -> pd.DataFrame:
        ...
        importance_df.to_csv(
            OUTPUT_DIR / "data" / "bulldog_feature_importance.csv", index=False
        )
        ...
        if SHAP_AVAILABLE and len(self.df_test) > 0:
            ...
            try:
                ...
                shap_importance.to_csv(
                    OUTPUT_DIR / "data" / "bulldog_shap_importance.csv", index=False
                )
    ...
    def analyze_clv(self) -> Dict:
        ...
        clv_stats = {
            ...
            "positive_clv_pct": (self.results_df["clv"] > 0).sum()
            / len(self.results_df)
            * 100,
            "negative_clv_pct": (self.results_df["clv"] < 0).sum()
            / len(self.results_df)
            * 100,
        ...
        # CLV by win/loss
        clv_by_result = self.results_df.groupby("result")["clv"].agg(
            ["mean", "std", "count"]
        )
        ...
        logger.info(
            f"  CLV-Profit Correlation: {clv_stats.get('clv_profit_correlation', 0):.3f}"
        )
    ...
    def analyze_variance(self) -> Dict:
        ...
        variance_stats = {
            ...
            "z_score": (
                (actual_profit - simulated_profits.mean()) / simulated_profits.std()
                if simulated_profits.std() > 0
                else 0
            ),
        }
        ...
        logger.info(
            f"  Simulated Mean: ${variance_stats['simulated_mean_profit']:,.2f}"
        )
        logger.info(f"  Z-Score: {variance_stats['z_score']:.2f}")
        logger.info(f"  P-Value: {variance_stats['p_value']:.4f}")
        logger.info(
            f"  Probability of Positive: {variance_stats['probability_positive']:.1%}"
        )
    ...
    def analyze_drawdown(self) -> Dict:
        ...
        drawdown_stats = {
            ...
            "avg_drawdown": (
                drawdowns[drawdowns < 0].mean() * 100 if (drawdowns < 0).any() else 0
            ),
            ...
            "worst_drawdown": (
                min([p["max_drawdown"] for p in drawdown_periods]) * 100
                if drawdown_periods
                else 0
            ),
        }
        ...
        logger.info(
            f"  Number of Drawdown Periods: {drawdown_stats['num_drawdown_periods']}"
        )
        logger.info(
            f"  Average Duration: {drawdown_stats['avg_drawdown_duration']:.1f} bets"
        )
    ...
    def analyze_statistical_significance(self) -> Dict:
        ...
        logger.info(
            f"  95% CI: [{stats_results['ci_95_lower']:.2f}%, {stats_results['ci_95_upper']:.2f}%]"
        )
