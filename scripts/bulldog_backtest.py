...
class BulldogBacktest:
    ...
    def __init__(
        self,
        model_path: str = "models/xgboost_improved.pkl",
        ...
        logger.info(
            f"Date range: {self.df['gameday'].min()} to {self.df['gameday'].max()}"
        )
    ...
    def _get_feature_columns(self) -> List[str]:
        ...
        feature_cols = [
            col for col in feature_cols if self.df[col].dtype in ["float64", "int64"]
        ]
    ...
    def _prepare_predictions(self):
        ...
        if model_features:
            ...
            available_features = [
                f for f in model_features if f in self.df_test.columns
            ]
            missing_features = [
                f for f in model_features if f not in self.df_test.columns
            ]

            if missing_features:
                logger.warning(
                    f"Model expects {len(missing_features)} features not in data, filling with 0: {missing_features[:5]}..."
                )
        ...
        # Actual outcomes
        self.df_test["actual"] = (
            self.df_test["home_score"] > self.df_test["away_score"]
        ).astype(int)
        ...
        if "home_moneyline" in self.df_test.columns:
            self.df_test["odds"] = self.df_test["home_moneyline"].apply(
                american_to_decimal
            )
        else:
    ...
    def run_backtest(
        ...
        # Initialize engine
        engine = BacktestEngine(initial_bankroll=self.initial_bankroll, config=config)
    ...
    def test_dimension_time_periods(self) -> Dict:
        ...
        results["full_period"] = self.run_backtest(
            self.df_test.copy(), name="Full Period (2020-2024)"
        )
        logger.info(
            f"  ✓ Complete: {results['full_period'].get('bet_count', 0)} bets, ROI: {results['full_period'].get('roi', 0):.2%}"
        )
        ...
        for season in [2020, 2021, 2022, 2023, 2024]:
            ...
            results[f"season_{season}"] = self.run_backtest(
                df_season, name=f"Season {season}"
            )
            logger.info(
                f"  ✓ Complete: {results[f'season_{season}'].get('bet_count', 0)} bets, ROI: {results[f'season_{season}'].get('roi', 0):.2%}"
            )
        ...
        results["early_season"] = self.run_backtest(
            df_early, name="Early Season (Weeks 1-9)"
        )
        results["late_season"] = self.run_backtest(
            df_late, name="Late Season (Weeks 10-18)"
        )
    ...
    def test_dimension_game_characteristics(self) -> Dict:
        ...
        if "is_dome" in self.df_test.columns:
            results["dome"] = self.run_backtest(
                self.df_test[self.df_test["is_dome"] == 1].copy(), name="Dome Games"
            )

        if "temp" in self.df_test.columns and "wind" in self.df_test.columns:
            df_good_weather = self.df_test[
                (self.df_test["temp"] > 50) & (self.df_test["wind"] < 10)
            ].copy()
            results["good_weather"] = self.run_backtest(
                df_good_weather, name="Good Weather (>50°F, wind <10 mph)"
            )

            df_bad_weather = self.df_test[
                (self.df_test["temp"] < 32) | (self.df_test["wind"] > 15)
            ].copy()
            results["bad_weather"] = self.run_backtest(
                df_bad_weather, name="Bad Weather (<32°F or wind >15 mph)"
            )
    ...
    def test_dimension_bet_sizing(self) -> Dict:
        ...
        for kf in kelly_fractions:
            ...
            results[f"kelly_{kf}"] = self.run_backtest(
                self.df_test.copy(), kelly_fraction=kf, name=f"Kelly Fraction {kf}"
            )
    ...
    def test_scenario_favorites_destroyer(self) -> Dict:
        ...
        if "spread_home" in self.df_test.columns:
            df_fav = self.df_test[
                (self.df_test["spread_home"] < -7) & (self.df_test["confidence"] > 0.65)
            ].copy()
        else:
            # Use odds instead
            df_fav = self.df_test[
                (self.df_test["odds"] < 1.7) & (self.df_test["confidence"] > 0.65)
            ].copy()
    ...
    def test_scenario_weather_wizard(self) -> Dict:
        ...
        if "wind" in self.df_test.columns and "temp" in self.df_test.columns:
            df_bad_weather = self.df_test[
                (self.df_test["wind"] > 15) | (self.df_test["temp"] < 32)
            ].copy()
    ...
    def save_results(self):
        ...
        if self.all_bets:
            ...
            all_bets_df.to_csv(
                OUTPUT_DIR / "data" / "bulldog_backtest_results.csv", index=False
            )
        ...
        if results_summary:
            ...
            results_df.to_csv(
                OUTPUT_DIR / "data" / "bulldog_performance_by_segment.csv", index=False
            )
...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="models/xgboost_improved.pkl",
    ...
    # Initialize backtest
    backtest = BulldogBacktest(model_path=args.model, data_path=args.data)

    # Filter to specific season if requested
    if args.test_season:
        logger.info(f"\nFiltering to season {args.test_season} only...")
        backtest.df_test = backtest.df_test[
            backtest.df_test["season"] == args.test_season
        ].copy()
