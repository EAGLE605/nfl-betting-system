...
class BulldogEdgeDiscovery:
    ...
    def load_data(self):
        ...
        try:
            ...
            logger.info(f"Date range: {self.data['season'].min()} to {self.data['season'].max()}")
    ...
    def test_basic_edges(self):
        ...
        # 2. REST ADVANTAGE
        if 'rest_days_home' in self.data.columns and 'rest_days_away' in self.data.columns:
    ...
    def test_epa_edges(self):
        ...
        if all(col in self.data.columns for col in ['epa_offense_home', 'epa_defense_away']):
            condition = (
        ...
        if all(col in self.data.columns for col in ['epa_offense_away', 'epa_defense_home']):
            condition = (
        ...
        if all(col in self.data.columns for col in ['epa_explosive_rate_home', 'epa_explosive_rate_away']):
            condition = (
            ...
            self.test_hypothesis(
                "Explosive Offense Present = OVER",
                condition,
                outcome
            )
    ...
    def test_situational_edges(self):
        ...
        if all(col in self.data.columns for col in ['injury_count_home', 'injury_count_away']):
    ...
    def test_seasonal_edges(self):
        ...
        if 'win_pct_home' in self.data.columns and 'win_pct_away' in self.data.columns:
            ...
            late_mismatch = condition & (self.data['win_pct_home'] > 0.6) & (self.data['win_pct_away'] < 0.4)
            outcome = self.data['home_win']
            self.test_hypothesis(
                "Late Season: Playoff Team vs Eliminated Team",
                late_mismatch,
                outcome
            )
    ...
    def test_combination_edges(self):
        ...
        if all(col in self.data.columns for col in ['rest_days_home', 'rest_days_away', 'post_bye_home']):
            rest_diff = self.data['rest_days_home'] - self.data['rest_days_away']
            condition = (
                (rest_diff >= 3) &
            ...
            outcome = self.data['home_win']
            self.test_hypothesis(
                "Perfect Storm: Home Post-Bye + 3+ Rest Advantage",
                condition,
                outcome
            )
            
        # 2. DOME + STRONG OFFENSE + WEAK DEFENSE
        if all(col in self.data.columns for col in ['is_dome', 'epa_offense_home', 'epa_defense_away']):
            condition = (
                (self.data['is_dome'] == 1) &
                (self.data['epa_offense_home'] > 0.1) &
                (self.data['epa_defense_away'] < -0.1)
            ...
            outcome = self.data['home_win']
            self.test_hypothesis(
                "Dome + Strong Home O vs Weak Away D",
                condition,
                outcome
            )
        ...
        if all(col in self.data.columns for col in ['div_game', 'elo_diff']):
            condition = (
                ...
                (self.data['elo_diff'] < -50)  # Home is underdog
            )
            outcome = self.data['home_win']
            self.test_hypothesis(
                "Divisional Game: Home Underdog",
                condition,
                outcome
            )
    ...
    def print_results(self):
        ...
        if not self.edges_found:
            logger.info("\nNO EDGES FOUND")
            logger.info("Market appears efficient. All tested hypotheses failed to show significant edge.")
            return
        ...
        for idx, edge in df.head(10).iterrows():
            ...
            logger.info(f"{'-'*80}")
            logger.info(f"Win Rate: {edge['win_rate']:.1%} ({edge['wins']}/{edge['sample_size']} bets)")
            logger.info(f"Edge: +{edge['edge']:.1%} above break-even")
            logger.info(f"Statistical Significance: {edge['significance']} (p={edge['p_value']:.4f})")
