...
class BulldogVisualizations:
    ...
    def __init__(self, results_path: str = None, bets_path: str = None):
        ...
        if results_path is None:
            results_path = Path("reports/bulldog_mode/data/bulldog_performance_by_segment.csv")
    ...
    def plot_equity_curve(self):
        ...
        main_bets = self.bets_df[self.bets_df['scenario'] == 'Full Period (2020-2024)'].copy()
        if len(main_bets) == 0:
        ...
        ax1.axhline(y=main_bets['bankroll'].iloc[0], color='r', linestyle='--', 
                   label='Initial Bankroll', alpha=0.7)
    ...
    def plot_win_rate_by_month(self):
        ...
        if 'gameday' in main_bets.columns:
            ...
            monthly_stats = main_bets.groupby('year_month').agg({
                'result': lambda x: (x == 'win').sum(),
                'bet_size': 'count'
            }).rename(columns={'result': 'wins', 'bet_size': 'total_bets'})
            monthly_stats['win_rate'] = monthly_stats['wins'] / monthly_stats['total_bets'] * 100
            
            fig, ax = plt.subplots(figsize=(14, 6))
            ax.bar(range(len(monthly_stats)), monthly_stats['win_rate'], 
                  color='#2E86AB', alpha=0.7)
            ax.axhline(y=50, color='r', linestyle='--', label='Break-even (50%)', alpha=0.7)
            ax.set_xlabel('Month', fontsize=12)
            ax.set_ylabel('Win Rate (%)', fontsize=12)
            ax.set_title('Win Rate by Month', fontsize=14, fontweight='bold')
            ax.set_xticks(range(len(monthly_stats)))
            ax.set_xticklabels([str(x) for x in monthly_stats.index], rotation=45, ha='right')
            ax.legend()
    ...
    def plot_feature_importance(self):
        ...
        importance_path = Path("reports/bulldog_mode/data/bulldog_feature_importance.csv")
        ...
        fig, ax = plt.subplots(figsize=(12, 8))
        ax.barh(range(len(top_20)), top_20['importance_pct'], color='#2E86AB', alpha=0.7)
    ...
    def plot_drawdown_analysis(self):
        ...
        ax.set_title('Underwater Chart - Drawdown Analysis', fontsize=14, fontweight='bold')
        ax.grid(alpha=0.3)
    ...
    def plot_bet_size_distribution(self):
        ...
        ax.hist(self.bets_df['bet_size'], bins=50, color='#2E86AB', alpha=0.7, edgecolor='black')
        ax.set_xlabel('Bet Size ($)', fontsize=12)
        ax.set_ylabel('Frequency', fontsize=12)
        ax.set_title('Bet Size Distribution', fontsize=14, fontweight='bold')
        ax.axvline(self.bets_df['bet_size'].mean(), color='r', linestyle='--', 
    ...
    def plot_win_loss_streaks(self):
        ...
        streaks = main_bets.groupby('streak').agg({
            'is_win': 'first',
            'bet_size': 'count'
        ...
        ax.bar(range(len(win_streaks)), win_streaks, color='green', alpha=0.7, label='Win Streaks')
        ax.bar(range(len(win_streaks), len(win_streaks) + len(loss_streaks)), 
              loss_streaks, color='red', alpha=0.7, label='Loss Streaks')
    ...
    def plot_confidence_calibration(self):
        ...
        calibration = self.bets_df.groupby('pred_bin').agg({
            'pred_prob': 'mean',
            'result': lambda x: (x == 'win').mean()
        }).dropna()
        
        fig, ax = plt.subplots(figsize=(10, 8))
        ax.scatter(calibration['pred_prob'], calibration['result'], 
    ...
    def plot_clv_distribution(self):
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.hist(self.bets_df['clv'] * 100, bins=50, color='#2E86AB', alpha=0.7, edgecolor='black')
        ax.axvline(0, color='r', linestyle='--', linewidth=2, label='Zero CLV')
        ax.axvline(self.bets_df['clv'].mean() * 100, color='g', linestyle='--', 
                  label=f'Mean: {self.bets_df["clv"].mean()*100:.2f}%')
        ax.set_xlabel('CLV (%)', fontsize=12)
        ax.set_ylabel('Frequency', fontsize=12)
        ax.set_title('Closing Line Value (CLV) Distribution', fontsize=14, fontweight='bold')
        ax.legend()
        ax.grid(alpha=0.3, axis='y')
    ...
    def plot_risk_return_scatter(self):
        ...
        scatter = ax.scatter(relevant['max_drawdown'], relevant['roi'], 
                           s=relevant['total_bets']*2, alpha=0.6, 
                           c=relevant['sharpe_ratio'], cmap='viridis')
        
        ax.set_xlabel('Max Drawdown (%)', fontsize=12)
        ax.set_ylabel('ROI (%)', fontsize=12)
        ax.set_title('Risk-Return Scatter (Size = # Bets, Color = Sharpe)', 
