...
class ModelEvolver:
    ...
    def create_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # EPA differentials
        epa_cols = ['epa_offense_home', 'epa_defense_home', 
                   'epa_offense_away', 'epa_defense_away']
        ...
        if 'temp' in df.columns:
            df['temp_effect'] = np.where(df['temp'] < 32, 1, 
                                        np.where(df['temp'] > 80, -1, 0))
    ...
    def optimize_hyperparameters(self, X_train, y_train, X_val, y_val):
        ...
        param_combinations = [
            {'n_estimators': 300, 'max_depth': 4, 'learning_rate': 0.02, 'min_child_weight': 5, 'subsample': 0.9, 'colsample_bytree': 0.9, 'gamma': 0.2, 'reg_alpha': 0.5, 'reg_lambda': 2},
            {'n_estimators': 300, 'max_depth': 5, 'learning_rate': 0.01, 'min_child_weight': 7, 'subsample': 0.8, 'colsample_bytree': 0.8, 'gamma': 0.1, 'reg_alpha': 0.1, 'reg_lambda': 1},
            {'n_estimators': 200, 'max_depth': 3, 'learning_rate': 0.05, 'min_child_weight': 3, 'subsample': 0.9, 'colsample_bytree': 0.9, 'gamma': 0.2, 'reg_alpha': 0.5, 'reg_lambda': 2},
            {'n_estimators': 300, 'max_depth': 4, 'learning_rate': 0.02, 'min_child_weight': 5, 'subsample': 0.85, 'colsample_bytree': 0.85, 'gamma': 0.15, 'reg_alpha': 0.3, 'reg_lambda': 1.5},
        ]
        ...
        for params in param_combinations:
            try:
                ...
                high_conf_mask = (y_pred_proba >= high_conf_threshold) | (y_pred_proba <= (1 - high_conf_threshold))
                
                if high_conf_mask.sum() > 10:  # Need at least 10 high-confidence predictions
                    y_pred_binary = (y_pred_proba >= 0.5).astype(int)
                    high_conf_correct = ((y_pred_binary == y_val.values) & high_conf_mask).sum()
                    high_conf_total = high_conf_mask.sum()
                    high_conf_win_rate = high_conf_correct / high_conf_total if high_conf_total > 0 else 0
                    
                    if high_conf_win_rate > best_score:
                        best_score = high_conf_win_rate
                        best_params = params.copy()
                        logger.info(f"New best: {high_conf_win_rate:.3f} win rate at {high_conf_threshold} threshold")
    ...
    def train_evolved_model(self, train_years: List[int] = [2022, 2023], 
                           test_year: int = 2024):
        ...
        logger.info("-" * 70)
        logger.info(f"{'Threshold':<12} {'Bets':<8} {'Wins':<8} {'Win Rate':<12} {'Coverage':<12}")
        logger.info("-" * 70)
        ...
        for threshold in thresholds:
            ...
            if high_conf_mask.sum() > 0:
                ...
                logger.info(f"{threshold:<12.2f} {total:<8} {wins:<8} {win_rate:<12.3f} {coverage:<12.3f}")
        ...
        if best_threshold:
            ...
            logger.info(f"✅ Best win rate: {best_win_rate:.3f} ({best_win_rate*100:.1f}%)")
...
def main():
    ...
    if win_rate and win_rate >= 0.75:
        ...
        logger.info(f"   Current win rate: {win_rate*100:.1f}%" if win_rate else "   No high-confidence predictions found")
        logger.info("   Next steps: Feature engineering, more data, or threshold adjustment")
