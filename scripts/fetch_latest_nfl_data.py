        ...
        try:
            ...
            for event in events:
                ...
                if game_info:
                    ...
                    print(
                        f"[GAME] {away} @ {home} | {away_score}-{home_score} | {status}"
                    )
...
def main():
    ...
    if games:
        ...
        with open(output_file, "w") as f:
            json.dump(
                {"fetched_at": datetime.now().isoformat(), "games": games}, f, indent=2
            )
        ...
        in_progress = sum(
            1 for g in games if g["status"] in ["STATUS_IN_PROGRESS", "In Progress"]
        )
        scheduled = sum(
            1 for g in games if g["status"] in ["STATUS_SCHEDULED", "Scheduled"]
        )
