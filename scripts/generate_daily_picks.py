...
class DailyPicksGenerator:
    ...
    def __init__(self, 
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Favorites-only model not found, using: {fallback_path}")
            except FileNotFoundError:
    ...
    def get_team_recent_stats(self, team: str, season: int = 2024) -> Dict:
        ...
        team_data = self.features_df[
            (self.features_df['home_team'] == team) | 
            (self.features_df['away_team'] == team)
        ].sort_values('gameday', ascending=False).head(4)
        ...
        stats = {
            ...
            'elo_rating': team_data['elo_home'].iloc[0] if not team_data.empty else 1500,
    ...
    def predict_game(self, home_team: str, away_team: str, 
                     weather: Dict = None, odds: Dict = None) -> Dict:
        """
        ...
        # This is a placeholder - actual features would come from feature pipeline
        features = {
            'elo_diff': home_stats.get('elo_rating', 1500) - away_stats.get('elo_rating', 1500),
            'home_advantage': 48,  # Historical average
            'rest_days': 7,  # Default
        }
        ...
        if weather:
            ...
            # Weather penalty
            if wind_speed > 15:
                features['weather_penalty'] = -3.0
            elif temp < 25:
                features['weather_penalty'] = -1.5
            else:
                features['weather_penalty'] = 0.0
        
        # Make prediction (placeholder - would use actual feature columns)
        # For now, use Elo-based probability
        ...
        # Add betting recommendation if odds available
        if odds:
    ...
    def _calculate_edge(self, prediction: Dict, odds: Dict) -> Dict:
        ...
        return {
            ...
            'best_bet': prediction['home_team'] if home_edge > 0.05 else (
                prediction['away_team'] if away_edge > 0.05 else None
            )
        }
    
    def generate_pick(self, game: Dict, prediction: Dict, 
                     line_shopping_report: Dict) -> Dict:
        """
        ...
        edge = betting_edge.get('home_edge' if best_bet == prediction['home_team'] else 'away_edge', 0)
        ...
        # Kelly bet sizing
        win_prob = prediction['home_win_prob'] if is_home else prediction['away_win_prob']
        ...
        # Ensure minimum bet
        bet_size = max(bet_size, self.bankroll * 0.01) if bet_size > 0 else self.bankroll * 0.02
    ...
    def print_picks_report(self, picks: List[Dict]):
        ...
        for tier in ['S', 'A', 'B', 'C']:
            ...
            for i, pick in enumerate(tier_picks, 1):
                ...
                print(f"    Edge: {pick['edge']} | Win Prob: {pick['win_probability']} | Confidence: {pick['confidence']}")
                print(f"    Bet Size: {pick['bet_size']} ({pick['bet_size_pct']}) | Kelly: {pick['kelly_fraction']}")
        ...
        print(f"Total Risk: ${total_bet:.0f} ({(total_bet/self.bankroll)*100:.1f}% of bankroll)")
