...
def generate_dashboard():
    ...
    ax1.axhline(
        y=10000, color="r", linestyle="--", alpha=0.7, label="Starting Bankroll"
    )
    ...
    if "tier" in df.columns:
        tier_stats = df.groupby("tier").agg({"result": ["count", "sum"]}).reset_index()
        tier_stats["win_rate"] = (
            tier_stats[("result", "sum")] / tier_stats[("result", "count")] * 100
        )
        ax2.bar(
            tier_stats["tier"],
            tier_stats["win_rate"],
    ...
    if "sportsbook" in df.columns:
        book_stats = (
            df.groupby("sportsbook")
            .agg({"profit": "sum", "bet_size": "sum"})
            .reset_index()
        )
    ...
    roi = (
        (total_profit / df[bet_size_col].sum()) * 100
        if df[bet_size_col].sum() > 0
        else 0
    )
