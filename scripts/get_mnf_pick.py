...
def get_mnf_pick():
    ...
    mnf_game = None
    for game in games:
        if "Panthers" in game.get("away_team", "") and "49ers" in game.get(
            "home_team", ""
        ):
            mnf_game = game
            break
            ...
            if market["key"] == "spreads":
                for outcome in market["outcomes"]:
                    if outcome["name"] == mnf_game["home_team"]:
                        ...
                        print(
                            f"  {book_name}: {mnf_game['home_team']} {spread:+.1f} ({odds:+d})"
                        )
    ...
    print("=" * 80)
    print(
        f"API Calls Remaining: {api.session.headers.get('x-requests-remaining', 'N/A')}"
    )
    print("=" * 80)
