...
class LineShoppingEngine:
    ...
    def find_best_total(self, game: Dict, over_under: str) -> Tuple[str, float, int, float]:
        """
    ...
    def find_best_values(self, min_edge: float = 0.5) -> List[Dict]:
        ...
        for game in self.current_odds:
            ...
            if home_spread_edge and float(home_spread_edge.replace('pts', '')) >= min_edge:
...
def main():
    ...
    for i, game in enumerate(odds[:3], 1):  # Show first 3 games
        ...
        # Moneyline
        print("\nBEST MONEYLINE:")
        print(f"  {report['moneyline']['home']['team']}: {report['moneyline']['home']['odds']:+d} @ {report['moneyline']['home']['best_book']}")
        print(f"  {report['moneyline']['away']['team']}: {report['moneyline']['away']['odds']:+d} @ {report['moneyline']['away']['best_book']}")
        
        # Spread
        print("\nBEST SPREAD:")
        if report['spread']['home']['spread']:
            print(f"  {report['spread']['home']['team']} {report['spread']['home']['spread']:+.1f} ({report['spread']['home']['odds']:+d}) @ {report['spread']['home']['best_book']}")
            print(f"  {report['spread']['away']['team']} {report['spread']['away']['spread']:+.1f} ({report['spread']['away']['odds']:+d}) @ {report['spread']['away']['best_book']}")
        
        # Total
        print("\nBEST TOTAL:")
        if report['total']['over']['total']:
            print(f"  Over {report['total']['over']['total']} ({report['total']['over']['odds']:+d}) @ {report['total']['over']['best_book']}")
            print(f"  Under {report['total']['under']['total']} ({report['total']['under']['odds']:+d}) @ {report['total']['under']['best_book']}")
    ...
    if opportunities:
        ...
        for opp in opportunities:
            if opp['type'] == 'spread':
                print(f"  - {opp['game']}: {opp['team']} {opp['edge']} edge @ {opp['best_book']}")
