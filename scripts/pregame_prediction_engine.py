...
class OddsAPIClient:
    ...
    def find_best_odds(
        self, game_odds: Dict, bet_type: str, team: str
...
class PreGameEngine:
    ...
    def __init__(self, model_path: str = "models/calibrated_model.pkl"):
        ...
        self.odds_client = (
            OddsAPIClient(self.odds_api_key) if self.odds_api_key else None
        )
    ...
    def generate_features(self, game_info: Dict) -> Dict:
        ...
        """
        logger.info(
            f"Generating features for {game_info['away_team']} @ {game_info['home_team']}"
        )
    ...
    def _find_game_odds(self, all_odds: List[Dict], game_info: Dict) -> Optional[Dict]:
        ...
        for game in all_odds:
            if (
                game["home_team"] in game_info["home_team"]
                or game_info["home_team"] in game["home_team"]
            ):
                return game
        return None

    def _generate_recommendations(
        self, prediction: Dict, odds: Optional[Dict], edges: List[Dict], game_info: Dict
    ) -> List[Dict]:
        """
                        ...
                        recommendations.append(
                            {
                                ...
                                "confidence_tier": (
                                    "S" if best_edge["significance"] == "High" else "A"
                                ),
                            }
                        )
            else:
...
def main():
    ...
    parser.add_argument(
        "--all-today", action="store_true", help="Analyze all games today"
    )
    parser.add_argument(
        "--output", default="reports/pregame_analysis.json", help="Output file path"
    )
    parser.add_argument(
