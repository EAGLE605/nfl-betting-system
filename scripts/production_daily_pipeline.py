    ...
    def get_current_week(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'season_type': data['season']['type'],  # 1=preseason, 2=regular, 3=postseason
        ...
        for game in games:
            ...
            if alert_time > now:
                alerts.append({
                    ...
                    'minutes_until_alert': int((alert_time - now).total_seconds() / 60)
                })
...
def main():
    ...
    for day in ['Thursday', 'Friday', 'Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday']:
    ...
    for alert in alerts[:10]:  # Show first 10
        print(f"  {alert['alert_time_str']}: {alert['away_team']} @ {alert['home_team']}")
    ...
    print(f"  - Will run prediction pipeline 1 hour before each game")
    print(f"  - Notifications will be sent with bet recommendations")
    print("\n" + "="*80 + "\n")
