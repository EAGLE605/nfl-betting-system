...
def install_requirements():
    ...
    requirements_files = ["requirements.txt", "dashboard/requirements.txt"]
...
def run_continuous_backtests():
    ...
    try:
        ...
        if (success_count + fail_count) > 0:
            print(
                f"  Overall Success Rate: {success_count/(success_count+fail_count)*100:.1f}%"
            )
    except Exception as e:
...
def main():
    ...
    print(
        f"{Colors.BOLD}{Colors.BLUE}NFL Betting System - Sandbox Setup & Backtest{Colors.END}"
    )
