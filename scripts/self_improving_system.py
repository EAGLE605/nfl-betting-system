...
class SelfImprovingSystem:
    ...
    def should_retrain_model(self) -> bool:
        ...
        logger.info(f"Model is {age_days} days old with {len(completed)} new results - no retrain needed")
        return False
    ...
    def evaluate_model(self, model_path: str) -> Dict:
        ...
        try:
            ...
            # Run backtest
            from scripts.backtest import prepare_predictions, calculate_metrics
            ...
            feature_cols = [col for col in df.columns 
                           if col not in ['gameday', 'game_id', 'season', 'week', 
    ...
    def compare_models(self, new_model_path: str, old_model_path: str) -> bool:
        ...
        # Compare metrics
        logger.info("\n[3] COMPARISON:")
        logger.info(f"  Win Rate:  {old_metrics['win_rate']:.2f}% → {new_metrics['win_rate']:.2f}% "
                   f"({new_metrics['win_rate']-old_metrics['win_rate']:+.2f}%)")
        logger.info(f"  ROI:       {old_metrics['roi']:.2f}% → {new_metrics['roi']:.2f}% "
                   f"({new_metrics['roi']-old_metrics['roi']:+.2f}%)")
        logger.info(f"  Profit:    ${old_metrics['total_profit']:.0f} → ${new_metrics['total_profit']:.0f} "
        ...
        better_overall = (new_metrics['win_rate'] > old_metrics['win_rate'] and 
                         new_metrics['total_profit'] > old_metrics['total_profit'])
    ...
    def deploy_model(self, model_path: str):
        ...
        if production_path.exists():
            backup_path = self.models_dir / f'xgboost_improved_backup_{datetime.now().strftime("%Y%m%d")}.pkl'
