...
class NotificationManager:
    ...
    def __init__(self):
        ...
        if all([self.email_user, self.email_password, self.email_recipient]):
            self.email_sender = EmailSender(
                self.email_user, self.email_password, self.email_recipient
            )
    ...
    def send_all_notifications(
        self, analysis_file: str, parlays_file: str
    ) -> Dict[str, bool]:
        """
        ...
        for analysis in analyses:
            if not analysis.get("recommendations"):
                logger.info(
                    f"No recommendations for {analysis['game_info']['away_team']} @ {analysis['game_info']['home_team']} - skipping"
                )
                continue
...
def main():
    ...
    parser.add_argument(
        "--analysis", required=True, help="Path to pregame_analysis.json"
    )
    parser.add_argument("--parlays", required=True, help="Path to parlays.json")
