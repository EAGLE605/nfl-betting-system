...
class GrokClient(BaseLLMClient):
    ...
    async def generate(self, prompt: str, system: str = "") -> str:
        ...
        response = await self.client.post(
            ...
            json={
                "model": self.config.model
                or "grok-3",  # Updated from deprecated grok-beta
                "messages": messages,
                "temperature": self.config.temperature,
                "max_tokens": self.config.max_tokens,
                "stream": False,
...
class LLMCouncil:
    ...
    def add_member(self, member: CouncilMember):
        ...
        logger.info(
            f"Added council member: {member.member_id} ({member.config.provider.value})"
        )
...
"""
    ...
    async def _get_member_analysis(
        self, member: CouncilMember, game_data: Dict
    ...
    async def analyze_game(self, game_data: Dict) -> CouncilDecision:
        ...
        avg_confidence = (
            sum(vote_confidence[winning_pick[0]])
            / len(vote_confidence[winning_pick[0]])
            if vote_confidence[winning_pick[0]]
            else 0
        )
        avg_edge = (
            sum(vote_edge[winning_pick[0]]) / len(vote_edge[winning_pick[0]])
            if vote_edge[winning_pick[0]]
            else 0
        )
        ...
        for analysis in valid_analyses:
            if analysis.pick == winning_pick[0]:
                ...
                dissenting_views.append(
                    f"{analysis.member_id}: {analysis.pick} ({analysis.reasoning[0] if analysis.reasoning else 'No reason'})"
                )
