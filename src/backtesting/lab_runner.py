...
class LabRunner:
    ...
    def run_walk_forward(
        ...
        try:
            ...
            schedules_df["week_num"] = (
                pd.to_datetime(schedules_df["gameday"]).dt.isocalendar().week
            )
            ...
            for i, window_start in enumerate(
                range(0, len(unique_weeks) - test_weeks, test_weeks)
            ):
                ...
                predictions_df = self.pred_generator.generate_predictions(
                    test_df, {"model_name": model_name}, pbp_df
                )
            ...
            if window_results:
                ...
                avg_win_rate = sum(w["win_rate"] for w in window_results) / len(
                    window_results
                )
                consistency = sum(1 for w in window_results if w["roi"] > 0) / len(
                    window_results
                )
            else:
                avg_roi = 0
...
class AsyncLabRunner:
    ...
    async def run_backtest(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        return await loop.run_in_executor(
            None, lambda: self._runner.run_backtest(*args, **kwargs)
        )
    ...
    async def run_comparison_backtest(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, lambda: self._runner.run_comparison_backtest(*args, **kwargs)
        )


# Convenience function for quick backtests
def quick_backtest(
    model_name: str, seasons: List[int] = None, bankroll: float = 10000
) -> Dict[str, Any]:
    """
