...
class NFLDataPipeline:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized NFLDataPipeline (data_dir={self.data_dir}, strict_mode={strict_mode})"
        )

    def _should_download(self, filepath: Path, seasons: List[int]) -> bool:
        """
        ...
        if file_age > timedelta(days=self.cache_days):
            logger.info(
                f"↻ Cache expired for {filepath.name} ({file_age.days} days old)"
            )
            return True
    ...
    def _check_file_integrity(
        self, filepath: Path, expected_rows: Optional[int] = None
    ) -> bool:
        """
    ...
    def _retry_download(
        self, download_func: callable, data_type: str, **kwargs
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.info(
                    f"Downloading {data_type} (attempt {attempt}/{self.max_retries})..."
                )
                ...
                logger.error(
                    f"✗ Download failed (attempt {attempt}/{self.max_retries}): {e}"
                )
    ...
    def get_schedules(
        self, seasons: List[int], force_download: bool = False
    ) -> pd.DataFrame:
        """
    ...
    def get_weekly_stats(
        ...
        """
        filepath = (
            self.raw_dir / f"weekly_{stat_type}_{min(seasons)}_{max(seasons)}.parquet"
        )
    ...
    def validate_data(
        ...
        if not null_cols.empty:
            logger.warning(f"⚠ {data_type} has nulls in critical columns:\n{null_cols}")
    ...
    def download_all(
        self, seasons: List[int], include_pbp: bool = True, force_download: bool = False
    ...
    def _save_metadata(
        self, seasons: List[int], results: Dict[str, pd.DataFrame]
    ) -> None:
        """
