...
class FormFeatures(FeatureBuilder):
    ...
    def build(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["point_diff_home"] = self._rolling_point_diff(
            df, "home_team", "home_point_diff"
        )

        # Calculate rolling averages for away team
        df["win_pct_away"] = self._rolling_win_pct(df, "away_team", "away_win")
        df["point_diff_away"] = self._rolling_point_diff(
            df, "away_team", "away_point_diff"
        )

        # Clean up temporary columns
        df = df.drop(
            columns=["home_win", "away_win", "home_point_diff", "away_point_diff"],
            errors="ignore",
        )

        logger.info(
            f"✓ Form features created: {len(self.get_feature_names())} features"
        )

        return df

    def _rolling_win_pct(
        self, df: pd.DataFrame, team_col: str, win_col: str
    ) -> pd.Series:
    ...
    def _rolling_point_diff(
        self, df: pd.DataFrame, team_col: str, diff_col: str
    ) -> pd.Series:
    ...
    def get_feature_names(self) -> List[str]:
        return ["win_pct_home", "win_pct_away", "point_diff_home", "point_diff_away"]
