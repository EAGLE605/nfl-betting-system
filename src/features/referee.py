...
class RefereeFeatures(FeatureBuilder):
    ...
    def build(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing:
            logger.warning(f"Missing columns for referee features: {missing}, using defaults")
        ...
        logger.info(f"✓ Referee features created: {len(self.get_feature_names())} features")
