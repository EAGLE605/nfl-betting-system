...
class WeatherFeatures(FeatureBuilder):
    ...
    def build(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'temp' in df.columns:
            df['temp'] = df['temp'].fillna(df['temp'].median() if df['temp'].notna().any() else 65)
        else:
            df['temp'] = 65.0  # Default moderate temperature
        
        # Wind
        if 'wind' in df.columns:
            df['wind'] = df['wind'].fillna(df['wind'].median() if df['wind'].notna().any() else 5)
        else:
        ...
        if 'roof' in df.columns:
            ...
            dome_stadiums = [
                'Mercedes-Benz Superdome', 'AT&T Stadium', 'Lucas Oil Stadium',
                'Ford Field', 'U.S. Bank Stadium', 'NRG Stadium', 'SoFi Stadium'
            ]
        ...
        logger.info(f"✓ Weather features created: {len(self.get_feature_names())} features")
