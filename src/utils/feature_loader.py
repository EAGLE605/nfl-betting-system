...
def get_feature_columns(
    ...
    if use_recommended:
        ...
        if rec_path.exists():
            try:
                ...
                logger.warning(
                    f"Could not load recommended features: {e}, using fallback"
                )
    ...
    numeric_features = (
        df[feature_cols].select_dtypes(include=["float64", "int64"]).columns.tolist()
    )
...
def validate_features(
    model_features: List[str], data_features: List[str], model_name: str = "model"
) -> bool:
    """
    ...
    if missing:
        logger.error(
            f"{model_name} expects {len(missing)} features not in data: {list(missing)[:5]}..."
        )
        return False

    if extra:
        logger.warning(
            f"Data has {len(extra)} features not in {model_name}: {list(extra)[:5]}..."
        )
