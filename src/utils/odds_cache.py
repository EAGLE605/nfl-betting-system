...
class CacheStats:
    ...
    def to_dict(self) -> Dict:
        return {
            **asdict(self),
            'hit_rate_pct': f"{self.hit_rate:.1f}%"
        }
...
class OddsCache:
    ...
    def get(self, key: str = "nfl_odds", max_age_minutes: Optional[int] = None) -> Optional[Dict]:
        """
        ...
        if self.enable_memory and key in self._memory:
            age_seconds = (datetime.now() - self._memory_timestamps[key]).total_seconds()
        ...
        if self.enable_files:
            ...
            if cached_file:
                try:
                    ...
                    age_minutes = (datetime.now() - datetime.fromtimestamp(cached_file.stat().st_mtime)).seconds // 60
                    self.stats.cache_hits += 1
        ...
        if self.enable_files:
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(f"Could not create symlink (not critical): {symlink_error}")
    ...
    def _find_recent_cache_file(self, key: str, max_age_minutes: Optional[int] = None) -> Optional[Path]:
        """
        ...
        cache_files = sorted(self.cache_dir.glob(pattern), key=lambda p: p.stat().st_mtime, reverse=True)
            ...
            for game in games:
                ...
                if commence_str:
                    try:
                        ...
                        commence_time = commence_time.replace(tzinfo=None)  # Remove timezone for comparison
                        time_to_game = (commence_time - now).total_seconds() / 3600  # Hours
    ...
    def update_api_usage(self, calls_remaining: int, response_time_ms: Optional[int] = None):
        """
    ...
    def clear_files(self, older_than_hours: int = 24):
        ...
        for filepath in self.cache_dir.glob("*.json"):
            if filepath.name.endswith("_latest.json") or filepath.name == "cache_stats.json":
                continue
