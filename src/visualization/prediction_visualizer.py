...
class PredictionVisualizer:
    ...
    def get_team_colors(self, team_name: str) -> Tuple[str, str]:
        ...
        colors = NFL_COLORS.get(
            team_name, {"primary": "#333333", "secondary": "#666666"}
        )
    ...
    def create_matchup_card(
        ...
        if reasoning:
            reason_text = (
                reasoning[0][:50] + "..." if len(reasoning[0]) > 50 else reasoning[0]
            )
            ax.text(
                5,
                0.4,
        ...
        if save_path:
            ...
            file_path = (
                self.output_dir / f"pick_{away_team}_{home_team}_{timestamp}.png"
            )
    ...
    def _draw_gauge(
        self, ax, x: float, y: float, value: float, label: str, max_val: float
    ):
    ...
    def create_equity_curve(
        ...
        fig.savefig(
            file_path, dpi=150, bbox_inches="tight", facecolor=self.background_color
        )
        plt.close(fig)
    ...
    def create_daily_picks_summary(
        self, picks: List[Dict[str, Any]], date: str, save_path: Optional[str] = None
    ) -> PickVisualization:
        """
        ...
        if n_picks == 0:
            ...
            fig, ax = plt.subplots(
                figsize=(12, fig_height), facecolor=self.background_color
            )
            ax.set_facecolor(self.card_color)
            ax.set_xlim(0, 12)
        ...
        file_path = (
            ...
            / f"daily_summary_{datetime.now().strftime('%Y%m%d')}.png"
        )
    ...
    def create_performance_dashboard(
        ...
        metrics_data = [
            ...
            (
                "PROFIT",
                f"${metrics.get('total_profit', 0):.0f}",
        ...
        if recent:
            x = range(len(recent))
            colors = [
                "#00FF00" if r == "W" else "#FF4444" if r == "L" else "#888888"
                for r in recent
            ]
        ...
        plt.suptitle(
            "PERFORMANCE DASHBOARD", color="white", fontsize=16, fontweight="bold"
        )
