...
class TestNFLDataPipeline:
    ...
    @patch('data_pipeline.nfl.import_schedules')
    def test_retry_download_success_after_failure(self, mock_import, pipeline, sample_schedules_df):
...
class TestIntegration:
    ...
    def test_data_quality_validation(self, temp_data_dir):
        ...
        assert expected_min <= len(schedules) <= expected_max, \
        ...
        # Check data types
        assert schedules['season'].dtype in ['int64', 'int32'], \
    ...
    def test_strict_mode_validation(self, temp_data_dir):
        ...
        pipeline = NFLDataPipeline(data_dir=temp_data_dir, strict_mode=True, max_retries=2)
    ...
    def test_download_all_components(self, temp_data_dir):
        ...
        seasons = [2023]
        results = pipeline.download_all(seasons, include_pbp=False)  # Skip PBP for speed
