...
class E2ETester:
    ...
    def test_complete_workflow(self):
        ...
        try:
            ...
            print(
                f"  [OK] Filtered {initial_count} -> {filtered_count} games ({filtered_count/initial_count*100:.1f}%)"
            )
            ...
            for _, row in filtered.iterrows():
                bet_size = kelly.calculate_bet_size(
                    row["pred_prob"], row["odds"], bankroll
                )
                if bet_size > 0:
                    bets.append(
                        {
            ...
            if bets:
                ...
                print(
                    f"      Total risk: ${total_risk:.2f} ({total_risk/bankroll*100:.1f}% of bankroll)"
                )
            ...
            assert all(
                1.3 < b["odds"] < 2.0 for b in bets
            ), "All bets should be favorites"
            assert all(b["bet_size"] > 0 for b in bets), "All bet sizes should be > 0"
            assert all(
                b["bet_size"] <= bankroll * 0.10 for b in bets
            ), "No bet should exceed 10%"
    ...
    def test_backtest_consistency(self):
        ...
        try:
            ...
            if history_path.exists():
                ...
                calculated_wins = (
                    (history["result"] == "win").sum()
                    if "result" in history.columns
                    else history["result"].sum()
                )
                calculated_rate = (calculated_wins / len(history)) * 100

                assert (
                    abs(calculated_rate - metrics["win_rate"]) < 1.0
                ), "Win rate mismatch"
                print("  [OK] Bet history consistent with metrics")
    ...
    def test_data_integrity(self):
        ...
        try:
            ...
            leak_features = [
                f for f in rec_features if any(kw in f.lower() for kw in leak_keywords)
            ]
            ...
            print("  [OK] All recommended features exist in data")

            # Check 3: No NaN in critical columns
            critical_cols = ["game_id", "gameday", "season"]
            for col in critical_cols:
                assert df[col].notna().all(), f"NaN values in {col}"
            print("  [OK] No NaN in critical columns")

            # Check 4: Temporal ordering
            df["gameday"] = pd.to_datetime(df["gameday"])
            assert (
                df["gameday"].is_monotonic_increasing
                or df.groupby("season")["gameday"].is_monotonic_increasing.all()
            ), "Data not temporally ordered"
            print("  [OK] Data is temporally ordered")
