...
class SandboxTester:
    ...
    def test_model_loading(self):
        ...
        try:
            ...
            print(f"  [OK] Model prediction works: {proba[0] if proba.ndim == 1 else proba[0, 1]:.4f}")
        except Exception as e:
    ...
    def test_kelly_criterion(self):
        ...
        print(f"  [OK] No edge correctly returns 0")
        
        # Test 4: Hot streak bonus
        recent_perf = {'win_rate_last_10': 0.80}
        bet_size_with_bonus = kelly.calculate_bet_size(0.70, 1.80, bankroll, recent_perf)
    ...
    def test_favorites_filter(self):
        ...
        test_data = pd.DataFrame({
            ...
            'odds': [1.40, 1.60, 1.90, 2.10, 2.50],  # Mix of favorites and underdogs
        ...
        print(f"  [OK] All odds in range 1.3-2.0: {all((filtered['odds'] > 1.3) & (filtered['odds'] < 2.0))}")
    ...
    def test_data_pipeline(self):
        ...
        print(f"  [OK] Features file exists")
        
        # Load and validate
        df = pd.read_parquet(features_path)
        assert len(df) > 0, "Dataframe is empty"
        assert 'target' not in df.columns or df['target'].notna().any(), "Target column issues"
        ...
        if has_leakage:
            print(f"  [WARN] Betting line columns present (may be OK if excluded from features)")
        else:
            print(f"  [OK] No betting line columns (good - no leakage)")
    ...
    def test_feature_engineering(self):
        ...
        if rec_path.exists():
            ...
            leak_features = [f for f in rec_features if any(x in f.lower() for x in ['moneyline', 'spread', 'odds', 'line'])]
            if leak_features:
                print(f"  [WARN] Potential leakage features: {leak_features[:3]}")
            else:
                print(f"  [OK] No betting line features in recommendations")
        else:
            print(f"  [WARN] Recommended features file not found")
    ...
    def test_backtest_engine(self):
        ...
        if metrics_path.exists():
            ...
            print(f"  [OK] Backtest metrics loaded")
            print(f"     Win Rate: {metrics['win_rate']:.2f}%")
            print(f"     ROI: {metrics['roi']:.2f}%")
            print(f"     Sharpe: {metrics.get('sharpe_ratio', 0):.2f}")
        else:
            print(f"  [WARN] Backtest metrics not found (run backtest first)")
        ...
        if history_path.exists():
            ...
            print(f"  [WARN] Bet history not found")
    ...
    def test_daily_picks(self):
        ...
        try:
            ...
            print(f"  [OK] DailyPicksGenerator initialized")
            
            # Check model loaded
            if generator.model is not None:
                print(f"  [OK] Model loaded")
            else:
                print(f"  [WARN] Model not loaded (may be OK if file missing)")
            ...
            print(f"  [OK] All APIs initialized")
            
            # Test favorites filter flag
            assert generator.favorites_only == True, "Favorites-only flag not set"
            print(f"  [OK] Favorites-only filter enabled")
            
        except Exception as e:
            if "Model not found" in str(e) or "FileNotFoundError" in str(type(e).__name__):
                print(f"  [WARN] Model file missing (expected in some environments)")
            else:
                raise
    ...
    def test_performance_dashboard(self):
        ...
        if history_path.exists():
            ...
            print(f"  [WARN] Bet history not found (run backtest first)")
    ...
    def test_integration(self):
        ...
        if model_path.exists():
            ...
            print(f"  [OK] Prediction -> Kelly -> Filter pipeline works")
            print(f"     Prob: {dummy_prob:.2%}, Odds: {odds:.2f}, Bet: ${bet_size:.2f}")
        else:
            print(f"  [WARN] Model not found, skipping integration test")
        ...
        if features_path.exists():
            ...
            print(f"  [WARN] Features file not found")
    ...
    def test_edge_cases(self):
        ...
        print(f"  [OK] Zero bankroll handled correctly")
        ...
        print(f"  [OK] Low probability handled correctly")
        ...
        try:
            ...
            print(f"  [OK] Missing model handled gracefully")
        except Exception as e:
        ...
        print(f"  [OK] Empty filter handled correctly")
